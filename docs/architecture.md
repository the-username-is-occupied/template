...
| 4 | 4-уровневая каскадная иерархия бандлов (LSM-style) | Детально описано в `bundle-architecture.md`. Контент из любых источников попадает в `active_delta` (10k слов), который сбрасывается в `active_quarter` (120k), затем сливается в `frozen_half` (240k) и `frozen_full` (480k). Это позволяет обходить лимиты NLM и минимизировать время переиндексации при непрерывном стриминге. original_item атомарен: не разрезается между бандлами. |
